 ...
 * 	<= 0 - error occured
 */
int sendReceive(int sock, byte *commandBuff, int commandLen,
				byte *responceBuff, int responceBuffSize)
{
	printPackage(commandBuff, commandLen, OUT);

	// Send command
	send(sock, commandBuff, commandLen, 0);
	usleep(TIME_OUT);

	// Get responce
	int len = recv(sock, responceBuff, responceBuffSize, 0);
	if (len)
...
int initConnection(int sock)
{
	...
	byte buf[BSZ];
	int len = sendReceive(sock, (byte *)&initCmd, sizeof(initCmd), buf, BSZ);
	if (len)
		return checkResult_1b(buf, len);
...
int closeConnection(int sock)
{
	...
	byte buf[BSZ];
	int len = sendReceive(sock, (byte *)&byeCmd, sizeof(byeCmd), buf, BSZ);
	if (len)
		return checkResult_1b(buf, len);
...
int getU(int sock, P3V *U)
{
	...
	byte buf[BSZ];
	int len = sendReceive(sock, (byte *)&getUCmd, sizeof(getUCmd), buf, BSZ);
...
int getCosF(int sock, P3VS *C)
{
	...
	byte buf[BSZ];
	int len = sendReceive(sock, (byte *)&getCosCmd, sizeof(getCosCmd), buf, BSZ);
...
int getF(int sock, float *f)
{
	...
	byte buf[BSZ];
	int len = sendReceive(sock, (byte *)&getFCmd, sizeof(getFCmd), buf, BSZ);
...
int getA(int sock, P3V *A)
{
	...
	byte buf[BSZ];
	int len = sendReceive(sock, (byte *)&getACmd, sizeof(getACmd), buf, BSZ);
...
int getP(int sock, P3VS *P)
{
	...
	byte buf[BSZ];
	int len = sendReceive(sock, (byte *)&getPCmd, sizeof(getPCmd), buf, BSZ);
...
int getS(int sock, P3VS *S)
{
	...
	byte buf[BSZ];
	int len = sendReceive(sock, (byte *)&getSCmd, sizeof(getSCmd), buf, BSZ);
 ...
 * 	OK - means ok.
 */
int getW(int sock, PWV *W, int periodId, int month, int tariffNo)
{
	...
	byte buf[BSZ];
	int len = sendReceive(sock, (byte *)&getWCmd, sizeof(getWCmd), buf, BSZ);
